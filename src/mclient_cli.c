...
cli_server_connect(void)
{
	...
	if (connect
	    (socket_handle, (struct sockaddr *)&my_addr,
		...
		else
		{
			for (i = 0; i < strlen(param); i++)
			{
				...
				else
				{
					...
					hex_string[2] = '\0';
					debug_fine
					    ("mclient_cli:decoding hex %s\n",
					     hex_string);
					sscanf(hex_string, "%x", &the_char);
					debug_fine("mclient_cli:value %i",
						   the_char);
					buf[pos++] = the_char;
					// skip two more chars
					i += 2;
...
cli_decode_response(int socket_handle, char *buf, mclient_cmd * response)
{
	...
	while (param != NULL)
	{
		 ...
		 */
		if (strncmp(decoded_player_id, player_id, strlen(player_id)) ==
		    0)
		{
			/*
	...
	if (response->cmd != NULL)
	{
			 ...
			 == 0)
		{
			...
			if (cli_data.index_info == cli_data.index_playing)
			{
				...
				{
					debug
					    ("mclient_cli:Getting new cover art\n");
					/*
		...
		else if (strncmp("status", response->cmd, strlen("status")) ==
			 0)
		{
			debug("mclient:cli_parse_response: Found status.\n");
		}
		else if (strncmp("newsong", response->cmd, strlen("newsong"))
			 == 0)
		{
		...
		else if (strncmp("button", response->cmd, strlen("button")) ==
			 0)
		{
		...
		else if (strncmp("listen", response->cmd, strlen("listen")) ==
			 0)
		{
		...
		else if (strncmp("display", response->cmd, strlen("display"))
			 == 0)
		{
		...
		else if (strncmp("duration", response->cmd, strlen("duration"))
			 == 0)
		{
		...
		else if (strncmp("mode", response->cmd, strlen("mode")) == 0)
		{
			...
			if (strncmp("play", response->param[0], strlen("play"))
			    == 0)
			{
				debug
				    ("mclient:cli_parse_response: Found play.\n");
				mclient_display_state = PLAY;
/// ###                reset_mclient_hardware_buffer = 1;
			}
			else if (strncmp
				 ("stop", response->param[0],
				  strlen("stop")) == 0)
			{
				debug
				    ("mclient:cli_parse_response: Found stop.\n");
				mclient_display_state = STOP;
/// ###                reset_mclient_hardware_buffer = 1;
			}
			else if (strncmp
				 ("pause", response->param[0],
				  strlen("pause")) == 0)
			{
				debug
				    ("mclient:cli_parse_response: Found pause.\n");
				mclient_display_state = PAUSE;
			}
		}
		else if (strncmp("version", response->cmd, strlen("version"))
			 == 0)
		{
			debug("mclient:cli_parse_response: Found version.\n");
			{
				int slim_major, slim_minor, slim_dot,
				...
				printf
				    ("mclient:cli_parse_response: Found slimserver version:%d.%d.%d, need:%d.%d.%d\n",
				     ...
				     SLIMSERVER_VERSION_MINOR_1,
				     SLIMSERVER_VERSION_MINOR_2);
				slim_composit_ver =
				    (slim_major * 10000) + (slim_minor * 100) +
				    slim_dot;
			 ...
			 0)
		{
			...
			cli_data.album_name_for_cover_art[cli_data.
							  album_index_1_of_6_for_cover_art]
			    [50] = '\0';
			 ...
			 0)
		{
			...
			cli_data.artist_name_for_cover_art[cli_data.
							  album_index_1_of_6_for_cover_art]
			    [20] = '\0';
...
cli_parse_playlist(mclient_cmd * response)
{
		...
		if (cli_data.tracks == 1)
		{
			...
			if (cli_fullscreen_widget_state != STREAMING_RADIO)
			{
				...
				for (i = CLI_MAX_TRACKS; i >= 0; i--)
				{
					...
					debug
					    ("mclient_cli:Printing history:%d.\n",
					     i);
				}
	...
	else if (strncmp("title", response->param[0], strlen("title")) == 0)
	{
		...
		if (cli_data.tracks == 1)
		{
			...
			mvpw_menu_change_item(mclient_fullscreen, (void *)(1),
					      string);
		...
		else
		{
			    ...
			    (cli_userfocus_timeout > time(NULL)))
			{
				 ...
				 */
				if (cli_data.index_info !=
				    cli_data.index_playing)
				{
					err =
...
cli_parse_display(mclient_cmd * response)
{
	...
	else if ((char_ptr = strstr(response->param[0], "Browse Music")) !=
		 NULL)
	{
	...
	else if ((char_ptr = strstr(response->param[0], "Internet Radio")) !=
		 NULL)
	{
	 ...
	 */
		  ...
		  strstr(response->param[0], cli_data.album)) != NULL)
	{
	...
	case PLAYLIST:
		...
		if ((char_ptr = strstr(response->param[0], "(")) != NULL)
		{
			...
			if (cli_data.index_userfocus !=
			    old_fullscreen_userfocus_item)
			{
				old_fullscreen_userfocus_item =
				    cli_data.index_userfocus;
		 ...
		 */
		if (strncmp
		    ("Now playing", response->param[0],
		     strlen("Now playing")) == 0)
		{
			sprintf(string, "%s <- Now Playing",
				response->param[1]);
				...
				for (i = (CLI_MAX_TRACKS - 1); i >= 0; i--)
				{
					...
					mvpw_menu_change_item
					    (mclient_fullscreen,
					     ...
					     cli_data.title_history[i + 1]);
					debug
					    ("mclient_cli:Moving history:%d.\n",
					     i);
				}

				if (strlen(response->param[1]) >= 49)
				{
					strncpy(cli_data.title_history[0],
						response->param[1], 49);
					cli_data.title_history[0][49] = '\0';
				}
				else
				{
					strcpy(cli_data.title_history[0],
					       response->param[1]);
				}
			}
			else
...
cli_pick_starting_index(void)
{
	...
	if (cli_data_index > (CLI_MAX_TRACKS / 2))
	{
		...
		{
			cli_data.index_info =
			    (cli_data_index - (CLI_MAX_TRACKS / 2));
			/*
			 * But not more then a screen full from the end of the list of
			 * titles.
			 */
			if ((cli_data.tracks - cli_data.index_info) <=
			    CLI_MAX_TRACKS)
			{
				cli_data.index_info =
				    cli_data.tracks - (CLI_MAX_TRACKS + 1);
			}
		}
	}
}
