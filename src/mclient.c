...
send_packet(int s, char *b, int l)
{
	...
	if (sendto(s, b, l, 0, (const struct sockaddr *)&ina, sizeof(ina)) ==
	    -1)
	{
		if (debug)
		...
		if (remote_buttons.local_menu)
		{
			...
			else
			{
				...
				if (cli_data.col_for_cover_art >= 3)
				{
					...
					{
						cli_data.
					    	album_start_index_for_cover_art -= 6;
					}
					else
					{
						cli_data.
...
///        break;                  /* play */
	case MVPW_KEY_MENU:
		...
		else
		{
			...
			mvpw_set_key(mclient_sub_localmenu,
				     mclient_localmenu_callback);
		}
...
read_packet(int s)
{
	...
	else
	{
		...
		case 'l':
			...
			if (display)
			{
				receive_display_data(slimp3_display, recvbuf,
						     bytes_read);
			}
			break;
		case 's':
...
mclient_server_connect(void)
{
	...
	else
	{
		if (debug)
			printf
			    ("mclient:Was able to connect to descriptor endpoint:%d\n",
			     s);
	}
