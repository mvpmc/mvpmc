/*
 *  Copyright (C) 2005-2007, Paul Warren <pdw@ex-parrot.com>
 *  http://www.mvpmc.org/
 *
...
curses2ir(int key)
{
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .10);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .20);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .30);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .40);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .50);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .60);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .70);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .80);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
	    cli_data.album_start_index_for_cover_art = (cli_data.album_max_index_for_cover_art * .90);
	    // Only need to adjust the widget (user focus) unless we fall off edge.
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
	...
	if (remote_buttons.local_menu)
	{
		...
		else
		{
			...
			cli_data.
			album_max_index_for_cover_art - (6 - cli_data.album_start_index_for_cover_art);
		}
		cli_data.trigger_proc_for_cover_art = TRUE;
	    }
	    else
	...
	if (remote_buttons.local_menu)
	{
		...
		cli_data.album_start_index_for_cover_art += 6;
		if (cli_data.album_start_index_for_cover_art >= cli_data.album_max_index_for_cover_art)
		{
		    cli_data.album_start_index_for_cover_art %= cli_data.album_max_index_for_cover_art;
		}
		cli_data.trigger_proc_for_cover_art = TRUE;
	    }
	    else
...
///        break;                  /* play */
	...
	if (remote_buttons.local_menu)
	{
		...
		cli_data.album_start_index_for_cover_art =
		    cli_data.album_max_index_for_cover_art - (6 - cli_data.album_start_index_for_cover_art);
	    }
	    cli_data.trigger_proc_for_cover_art = TRUE;
	}
	else
...
receive_display_data(char *ddram, unsigned short *data, int bytes_read)
{
	...
	case 0x02:		/* command */
		...
		{
		    if (debug)
			printf("mclinet:UNDEFINED CONTROL CHARACTER!!! c:%-2.2x n:%-2.2d\n", c, n);
		}
		break;
	    }
	case 0x00:		/* delay */
