...
int mvp_server_init(void)
{
    ...
    if ( (surface = osd_create_surface(720,surface_y,0,OSD_DRAWING) )  == NULL ) {
        printf("Couldn't create surface\n");
        return -1;
    }
    if ( (surface2 = osd_create_surface(720,surface_y,0,OSD_DRAWING) )  == NULL ) {
        printf("Couldn't create surface\n");
        osd_destroy_surface(surface);
        surface = NULL;
        return -1;
    }
    if ( (surface_blank = osd_create_surface(720,surface_y,0,OSD_DRAWING) )  == NULL ) {
        printf("Couldn't create surface\n");
