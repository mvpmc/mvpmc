...
cmyth_proginfo_dup(cmyth_proginfo_t p)
{
	...
	ret->proginfo_unknown_2 = p->proginfo_unknown_2;
	cmyth_dbg(CMYTH_DBG_DEBUG, "%s }\n", __FUNCTION__);
	return ret;
}
...
delete_command(cmyth_conn_t control, cmyth_proginfo_t prog, char *cmd)
{
	...
	cmyth_datetime_to_string(originalairdate_dt,
				 prog->proginfo_originalairdate);
	...
	if (control->conn_version >= 25) {
		sprintf(buf,
			...
			prog->proginfo_timestretch,
			prog->proginfo_unknown_2); 
	}
	else if (control->conn_version >= 18) {
		sprintf(buf,
			"%s 0[]:[]"
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]%ld[]:[]"
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]%lld[]:[]"
	...
	else if (control->conn_version >= 15) {
		sprintf(buf,
			"%s 0[]:[]"
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]%ld[]:[]"
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]%lld[]:[]"
...
cmyth_proginfo_string(cmyth_proginfo_t prog)
{
	...
	if (prog->proginfo_version >= 25) {
		sprintf(ret,
			...
			prog->proginfo_hasairdate,
			prog->proginfo_unknown_2); }
		else if (prog->proginfo_version >= 18) {
		sprintf(ret,
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]%ld[]:[]"
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]"
			"%lld[]:[]%lld[]:[]%s[]:[]%s[]:[]%s[]:[]"
			...
			prog->proginfo_hasairdate,
			prog->proginfo_timestretch); }
	else if (prog->proginfo_version >= 15) {
		sprintf(ret,
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]%ld[]:[]"
			"%s[]:[]%s[]:[]%s[]:[]%s[]:[]"
			"%lld[]:[]%lld[]:[]%s[]:[]%s[]:[]%s[]:[]"
...
{
	if (!prog) {
		cmyth_dbg(CMYTH_DBG_ERROR, "%s: NULL timestretch\n",
			  __FUNCTION__);
		return NULL;
	}
